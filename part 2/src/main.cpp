...
int generalInfo()
{
	...
	case 4:
		...
		for (size_t i = 0; i < projects.size(); i++)
			cout << "\t\t" << projects[i] << " [" << user.getProjectCost(projects[i]) << "$]" << endl;
		cin.ignore(100, '\n');
		cin.get();
		generalInfo();
		break;
...
int manageOldAssociates()
{
	system("cls");
	hashAssociate oldAssociates = user.getOldAssociates();
	hashAssociate::iterator it;
	vector<Associate*> associates = user.getAssociates();
	vector<int> aProjects;
	Project proj;
	string name, password;
	int op, aIndex, i, op2;

	if (!oldAssociates.empty())
	{
		cout << "\n\tOld associates:\n";
		user.listOldAssociates();
	}
	...
	case 1:
		...
		cout << "\n\t> ";
		cin >> aIndex;
		while ((aIndex <= 0) || (aIndex > oldAssociates.size()))
		{
			cin.clear();
			cin.ignore(100, '\n');
			cout << "\n\t> ";
		...
		it = oldAssociates.begin();
		i = 1;
		while (it != oldAssociates.end() && i < aIndex)
		{
			it++;
			i++;
		}

		cout << "\n\tAssociate ID: " << (*it)->getID();
		cout << "\n\tName: " << (*it)->getName();
		cout << "\n\tType: " << (*it)->getType();
		aProjects = (*it)->getProjects();
		if (aProjects.size() > 0)
		{
			cout << "\n\tProjects:\n";
			for (size_t i = 0; i < aProjects.size(); i++)
	...
	case 2:
		...
		cout << "\n\t> ";
		cin >> aIndex;
		while ((aIndex <= 0) || (aIndex > associates.size() + 1))
		{
			cin.clear();
			cin.ignore(100, '\n');
			cout << "\n\t> ";
		...
		if (aIndex == associates.size() + 1)
			manageOldAssociates();

		user.addOldAssociate(associates[aIndex - 1]);
		user.removeAssociate(associates[aIndex - 1]->getID());

		user.saveAssociates("associates.txt");
		user.saveOldAssociates("oldAssociates.txt");
		manageOldAssociates();
		break;
	case 3:
		cout << "\n\t> ";
		...
		if ((*it)->getType() == "Programmer")
			user.updateOldAssociate(new Programmer((*it)->getID(), name, password, (*it)->getProjects()));
		else if ((*it)->getType() == "Designer")
			user.updateOldAssociate(new Designer((*it)->getID(), name, password, (*it)->getProjects()));
		else if ((*it)->getType() == "Manager")
			user.updateOldAssociate(new Manager((*it)->getID(), name, password, (*it)->getProjects()));
		else if ((*it)->getType() == "Tester")
			user.updateOldAssociate(new Tester((*it)->getID(), name, password, (*it)->getProjects()));
...
{
	system("cls");
	int op, cIndex, i, clientsNumber;
	vector<Client> clients;
	BST<Client> clients_bst = user.getClients();
	...
	if (!clients_bst.isEmpty())
	{
		cout << "\n\tClients:\n";
		clientsNumber = user.listClients();
	}
	...
	case 1:
		...
		cout << "\n\t> ";
		cin >> cIndex;
		while ((cIndex <= 0) || (cIndex > clientsNumber))
		{
			cin.clear();
			cin.ignore(100, '\n');
			cout << "\n\t> ";
	...
	case 3:
		...
		if (name == "")
			name = tmp.getName();
		client = Client(tmp.getID(), name, tmp.getProjects());
		user.updateClient(client);
		user.saveClients("clients.txt");
		manageClients();
		break;
...
int manageApplications()
{
	system("cls");
	priority_queue<Application> applications = user.getApplications(), tmp;
	int op, workingTime, skills, aIndex, i;
	bool pastAssociate;
	string name, wt, s, pa;
	...
	case 1:
		...
		cout << "\tPast associate: ";
		cin >> pastAssociate;
		application = Application(name, workingTime, skills, pastAssociate);
		user.addApplication(application);
		user.saveApplications("applications.txt");
		manageApplications();
		break;
	case 2:
		cout << "\n\t> ";
		cin >> aIndex;
		while ((aIndex <= 0) || (aIndex > applications.size()))
		{
			cin.clear();
			cin.ignore(100, '\n');
			cout << "\n\t> ";
		...
		getline(cin, name);
		if (name == "")
			name = tmp.top().getName();
		cout << "\tWorking time: ";
		getline(cin, wt);
		if (wt == "")
			workingTime = tmp.top().getWorkingTime();
		else
		{
			ss << wt;
			ss >> workingTime;
		...
		getline(cin, pa);
		if (pa == "")
			pastAssociate = tmp.top().getPastAssociate();
		else
		{
			ss << pa;
			ss >> pastAssociate;
		...
		application = Application(tmp.top().getID(), name, workingTime, skills, pastAssociate);
	...
	case 3:
		...
		user.removeApplication(tmp.top().getID());
		user.saveApplications("applications.txt");
		manageApplications();
		break;
	case 4:
...
bool Client::operator<(const Client& c1) const
{
	...
	for (size_t i = 0; i < projects.size(); i++)
	{
		...
		try
		{
			project = user.searchProjectByID(projects[i]);
		}
		catch (NotFound &e)
		{
			exception = true;
		...
		if (!exception)
			t1 += user.getProjectCost(project);
	}
	...
	for (size_t i = 0; i < p.size(); i++)
	{
		...
		try
		{
			project = user.searchProjectByID(p[i]);
		}
		catch (NotFound &e)
		{
			exception = true;
		...
		if (!exception)
			t2 += user.getProjectCost(project);
	}
	...
	if (t1 < t2)
		...
		return false;
	else if (projects.size() < p.size())
		return true;
	else if (projects.size() == p.size() && ID != c1.getID())
		return true;
	else
		return false;
